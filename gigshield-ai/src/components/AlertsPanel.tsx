...
const AlertCard: React.FC<{
  ...
  onDeleteAlert: (alertId: string) => void
  onEscalateAlert: (alertId: string) => void
}> = ({ alert, onResolveAlert, onDeleteAlert, onEscalateAlert }) => {
  const Icon = alertIcons[alert.type]
  const colors = priorityColors[alert.priority]
  const iconColor = priorityIconColors[alert.priority]

  return (
    <div className={clsx('rounded-xl p-3 border transition-all hover:scale-[1.01] cursor-default', colors.bg, colors.border)}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 mt-2">
            <button onClick={() => onResolveAlert(alert.id)} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25 transition-colors">
              <CheckCircle2 size={10} /> Resolve
            </button>
            <button onClick={() => onEscalateAlert(alert.id)} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold bg-amber-500/15 text-amber-400 hover:bg-amber-500/25 transition-colors">
              <Siren size={10} /> Escalate
            </button>
            <button onClick={() => onDeleteAlert(alert.id)} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold bg-red-500/15 text-red-400 hover:bg-red-500/25 transition-colors">
...
const AlertsPanel: React.FC<AlertsPanelProps> = ({ alerts, onResolveAlert, onDeleteAlert, onEscalateAlert }) => {
  return (
    <div className={clsx('card dark:bg-[#0d1528] bg-white dark:border-[#1a2540] border-gray-200 flex flex-col h-full')}>
